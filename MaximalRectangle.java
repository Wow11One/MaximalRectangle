...
public class MaximalRectangle{
    ...
    public static String[] matrixInput()
    {
        ...
        System.out.println("Hello! How many rows does the matrix have?");
        int matrixRowAmount = Integer.parseInt(scanner.nextLine());//get results
        //ask for input
        System.out.println("Enter all matrix rows on a new line:");
        String[] matrixResult = new String[matrixRowAmount];
        String matrixRow;
        int rowLength = 0;

        for(int i = 0; i < matrixRowAmount; i++)
        {
            matrixRow = scanner.nextLine();//get input
            if(i == 0)
                rowLength = matrixRow.length();//get length of a row
            inputValidation(matrixRow, rowLength);
            matrixResult[i] = matrixRow;
        }
     ...
     * @param rowLength length of the string to be checked
     */
    public static void inputValidation(String str, int rowLength)
    {
    ...
    /**
     * function that finds the largest rectangular area possible in a given histogram
     * where the largest rectangle can be made of a number of contiguous bars whose heights are given in an array.
     ...
     * The soultion is using stack to store bars.
     * @param histogram
     * @param n
     * @return
     */
